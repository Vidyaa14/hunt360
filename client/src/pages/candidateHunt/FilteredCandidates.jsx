...
const FilteredCandidates = () => {
    ...
    return (
        <div className="p-5 bg-gray-100 min-h-screen">
            <h2 className="text-2xl text-purple-800 mb-5 text-center">
                Filtered Candidates
            </h2>
            <div className="flex flex-wrap gap-5 justify-center">
                {candidates.length === 0 ? (
                    ...
                    candidates.map((candidate, index) => (
                        <div
                            key={index}
                            className="bg-white rounded-lg p-4 shadow-md w-72 flex flex-col gap-2"
                        >
                            ...
                            <p className="text-gray-600">
                                Sub Domain: {candidate.Sub_Domain}
                            </p>
                            <p
                                className={`font-bold ${candidate.email_status === 1 ? 'text-green-600' : 'text-red-600'}`}
                            >
                                Email Status:{' '}
                                {candidate.email_status === 1
                                    ? 'Sent'
                                    : 'Not Sent'}
                            </p>
                            <button
                                ...
                                onClick={() =>
                                    handleSendEmail(candidate, index)
                                }
                            >
                                {candidate.email_status === 1
                                    ? 'Email Sent'
