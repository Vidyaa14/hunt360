...
const DataScraping = () => {
    ...
    const handleScrape = async () => {
        ...
        try {
            ...
            setLogs((prevLogs) => [...prevLogs, "✅ Scraping completed successfully!"]);
            if (response.data.fileName) setFinalFile(response.data.fileName);
        } catch (error) {
            console.error("❌ Scraping Error:", error.message);
            setLogs((prevLogs) => [...prevLogs, "❌ Error occurred while scraping."]);
    ...
    return (
        <div className="min-h-screen px-4 sm:px-6 lg:px-8 py-4 sm:py-0 overflow-y-auto overflow-x-hidden" style={{ backgroundColor: '#ede7f6' }}>
            <div className="rounded-xl">
                {/* Scraping Configuration */}
                <div
                    className="py-4 sm:py-6 px-6 sm:px-8 rounded-md mb-4 sm:mb-6 shadow mx-4 sm:mx-6 lg:mx-[40px]"
                    style={{ backgroundColor: '#e6e0f8', marginTop: '64px', padding: '15px', marginBottom: '20px' }}
                >
                    <h2 className="text-lg sm:text-xl font-bold text-black">Scraping Configuration</h2>
                    <p className="text-xs sm:text-sm text-black">Enter criteria for scraping data.</p>
                </div>
                ...
                <div
                    ...
                    style={{ padding: '20px' }}
                >
                    <label className="block text-xs sm:text-sm font-bold text-[#4A148C] mb-1">Industry</label>
                    <input
                        type="text"
                    ...
                    <label className="block text-xs sm:text-sm font-bold text-[#4A148C] mb-1">City</label>
                    <input
                        type="text"
                        value={customState}
                    ...
                    <label className="block text-xs sm:text-sm font-bold text-[#4A148C] mb-1">Website</label>
                    <select
                    ...
                    >
                        ...
                        {websites.map((website, index) => (
                            <option key={index} value={website.value}>{website.name}</option>
                        ))}
                    </select>
                </div>
                ...
                <div className="mt-4 sm:mt-6 text-xs sm:text-sm text-gray-800 mx-4 sm:mx-6 lg:mx-[40px]">
                    {logs.length > 0
                        ...
                        : finalFile
                            ? <p>Scraped Data Saved: <strong>{finalFile}</strong></p>
                            : <p>No data available.</p>}
                </div>

                {/* Previous Scraped Data Section */}
                <div className="mx-4 sm:mx-6 lg:mx-[40px]">
                    <h3 className="text-base sm:text-lg font-semibold mt-6 sm:mt-8 mb-1 sm:mb-2">📌 Previous Scraped Data</h3>
                        ...
                        <table className="w-full border-collapse bg-[#f9f9f9] shadow-md text-xs sm:text-sm font-sans mt-4 sm:mt-5 min-w-[600px]">
                            <thead>
                                <tr className="bg-[#6a0dad]">
                                    <th className="p-2 sm:p-[12px] text-[#ffffff] uppercase text-left border border-[#ddd] text-xs sm:text-sm">Job Title</th>
                            ...
                            <tbody>
                                {scrapedData.length > 0 ? (
                                    scrapedData.map((row, index) => (
                                        <tr key={index} className={`${index % 2 === 0 ? 'bg-[#e6e6fa]' : ''} hover:bg-[#dcd0ff] transition duration-300`}>
                                            <td className="p-2 sm:p-[12px] border border-[#ddd]">{row.job_title}</td>
                                            <td className="p-2 sm:p-[12px] border border-[#ddd]">{row.company_name}</td>
                                            <td className="p-2 sm:p-[12px] border border-[#ddd]">{row.location}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
                                        <td colSpan="3" className="text-center p-3 sm:p-4 text-red-600">⚠️ No previous scrapes available.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
