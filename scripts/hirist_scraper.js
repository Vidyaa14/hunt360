...
(async function main() {
    ...
    try {
        ...
        const searchInput = await driver.findElement(
            By.xpath("//input[@placeholder='Search Jobs']")
        );
        ...
        while (!interrupted && noNewScrolls < 5) {
            ...
            const jobs = await driver.findElements(
                By.xpath("//*[contains(@class, 'job-description')]")
            );
            ...
            for (let job of jobs) {
                ...
                try {
                    const titleEl = await job.findElement(
                        By.css('div.job-description .job-title a')
                    );
                    ...
                    let company = 'Not Provided',
                        location = 'N/A';
                    try {
                        company = await driver
                            .findElement(
                                By.xpath(
                                    "//*[@id='observer-div']/div/div[3]/div[2]/div/div/div[2]/a[1]"
                                )
                            )
                            .getText();
                    } catch {}
                    try {
                        location = await driver
                            .findElement(By.xpath("//*[@id='location']"))
                            .getText();
                    } catch {}
                    ...
                    const mapsData = await getGoogleMapsData(
                        company,
                        selectedLocation,
                        driver
                    );
                    ...
                    console.log(
                        `Scraped: ${title} -> ${company} -> ${location} | GST: ${gstToSave}`
                    );
                    saveData();
                } catch (err) {
