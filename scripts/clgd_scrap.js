...
if (args.length !== 3) {
    console.error("Usage: node clgd_scrap.js <state_name> <CITY_NAME> <COURSE_NAME>");
    process.exit(1);
}
...
const excelFile = path.join(downloadsFolder, `${safe_STATE_NAME}_${safe_CITY_NAME}_${safe_COURSE_NAME}_${timestamp}.xlsx`);
...
(async () => {
    ...
    await page.goto("https://collegedunia.com/india-colleges", { waitUntil: "networkidle2" });
    await page.waitForTimeout(5000); // allow page to load

    // ---------- APPLY FILTERS ----------
    // 1. Filter by Course
    try {
        await page.waitForXPath("//button[@data-filter-toggle='course_tag_id']", { visible: true });
        const [courseFilterBtn] = await page.$x("//button[@data-filter-toggle='course_tag_id']");
        if (courseFilterBtn) {
            await courseFilterBtn.click();
            console.log("Clicked on 'Course' filter.");
        }
        // Wait for input field and type the course name
        await page.waitForXPath("//input[@placeholder='FIND COURSE']", { visible: true });
        const [courseInput] = await page.$x("//input[@placeholder='FIND COURSE']");
        await courseInput.type(COURSE_NAME);
        // Wait and click the li option that contains COURSE_NAME text
        await page.waitForXPath(`//li[contains(., '${COURSE_NAME}')]`, { visible: true });
        const [courseOption] = await page.$x(`//li[contains(., '${COURSE_NAME}')]`);
    ...
    // 2. Filter by State
    try {
        await page.waitForXPath("//button[@data-filter-toggle='state']", { visible: true });
        const [stateFilterBtn] = await page.$x("//button[@data-filter-toggle='state']");
        if (stateFilterBtn) {
            await stateFilterBtn.click();
            console.log("Clicked on 'State' filter.");
        }
        await page.waitForXPath("//input[@placeholder='FIND STATE']", { visible: true });
        const [stateInput] = await page.$x("//input[@placeholder='FIND STATE']");
        await stateInput.type(STATE_NAME);
        await page.waitForXPath(`//li[contains(., '${STATE_NAME}')]`, { visible: true });
        const [stateOption] = await page.$x(`//li[contains(., '${STATE_NAME}')]`);
    ...
    // 3. Filter by City
    try {
        await page.waitForXPath("//button[@data-filter-toggle='city']", { visible: true });
        const [cityFilterBtn] = await page.$x("//button[@data-filter-toggle='city']");
        if (cityFilterBtn) {
            await cityFilterBtn.click();
            console.log("Clicked on 'City' filter.");
        }
        await page.waitForXPath("//input[@placeholder='FIND CITIES']", { visible: true });
        const [cityInput] = await page.$x("//input[@placeholder='FIND CITIES']");
        await cityInput.type(CITY_NAME);
        await page.waitForXPath(`//li[contains(., '${CITY_NAME}')]`, { visible: true });
    ...
    while (scrollAttempts < scrollLimit) {
        ...
        validRows = await page.evaluate(() => {
            ...
            const nodes = Array.from(document.querySelectorAll("table.listing-table tbody tr"));
            return nodes.filter(node => node.querySelector("h3")).map(node => {
    ...
    const collegeData = await page.$$eval("table.listing-table tbody tr", (rows, fileName, COURSE_NAME, STATE_NAME, CITY_NAME) => {
        const data = [];
        rows.forEach(row => {
            ...
            if (row.querySelector("h3")) {
                try {
                    ...
                    const collegeName = row.querySelector("td:nth-child(2) h3")?.innerText.trim() || "";
                    // Course fees from third <td> with a span having class containing 'text-green'
                    const courseFees = row.querySelector("td:nth-child(3) span[class*='text-green']")?.innerText.trim() || "";
                    // For placement, get average and highest from fourth <td>.
                    const placementAvgElem = row.querySelector("td:nth-child(4) span[title='Average Package']") ?
                        row.querySelector("td:nth-child(4) span[title='Average Package']").previousElementSibling : null;
                    const placementHighestElem = row.querySelector("td:nth-child(4) span[title='Highest Package']") ?
                        row.querySelector("td:nth-child(4) span[title='Highest Package']").previousElementSibling : null;
                    const placementAvg = placementAvgElem ? placementAvgElem.innerText.trim() : "";
                    const placementHighest = placementHighestElem ? placementHighestElem.innerText.trim() : "";
                    const placement = `Average: ${placementAvg}, Highest: ${placementHighest}`;
                    // Ranking from sixth <td> (assumed structure based on your code)
                    const ranking = row.querySelector("td:nth-child(6) span.rank-span")?.innerText.trim() || "";
                    data.push({
                        File: fileName,
        ...
        return data;
    }, fileName, COURSE_NAME, STATE_NAME, CITY_NAME);
    ...
    async function getCollegeDetails(collegeName) {
        ...
        try {
            await mapPage.goto("https://www.google.com/maps", { waitUntil: "networkidle2" });
            await mapPage.waitForSelector("#searchboxinput", { visible: true });
            await mapPage.evaluate(() => { document.querySelector("#searchboxinput").value = ""; });
            ...
            const address = await mapPage.evaluate(() => {
                ...
                if (btn) {
                    return btn.getAttribute("aria-label").replace("Address: ", "").trim();
                }
                return "N/A";
            });
            ...
            const phone = await mapPage.evaluate(() => {
                ...
                if (btn) {
                    return btn.getAttribute("aria-label").replace("Phone: ", "").trim();
                }
                return "N/A";
            });
            await mapPage.close();
