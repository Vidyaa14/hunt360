...
const excelFile = path.join(
    downloadsFolder,
    `${safeState}_${safeDistrict}_${timestamp}.xlsx`
);
...
(async function scrape() {
    ...
    try {
        await driver.get(
            'https://dashboard.aishe.gov.in/hedirectory/#/institutionDirectory/universityDetails/C/ALL'
        );
        ...
        try {
            await driver.wait(
                until.elementLocated(By.className('loadermainbg')),
                10000
            );
            await driver.wait(
                until.elementIsNotVisible(
                    await driver.findElement(By.className('loadermainbg'))
                ),
                20000
            );
        } catch {
            await driver.executeScript(
                "document.querySelector('.loadermainbg').style.display='none';"
            );
        }
        ...
        const stateDropdown = await driver.wait(
            until.elementLocated(
                By.css('mat-select[formcontrolname="state1"]')
            ),
            15000
        );
        ...
        const stateOption = await driver.wait(
            until.elementLocated(
                By.xpath(`//mat-option/span[contains(., '${STATE_NAME}')]`)
            ),
            10000
        );
        ...
        const districtDropdown = await driver.wait(
            until.elementLocated(
                By.css('mat-select[formcontrolname="district1"]')
            ),
            15000
        );
        ...
        const districtOption = await driver.wait(
            until.elementLocated(
                By.xpath(`//mat-option/span[contains(., '${DISTRICT_NAME}')]`)
            ),
            10000
        );
        ...
        while (true) {
            ...
            if (data.length > 0) {
                console.log(
                    `✅ Page ${page++}: Scraped ${data.length} colleges`
                );
            ...
            const nextBtn = await driver.findElement(
                By.css('button.mat-mdc-paginator-navigation-next')
            );
        ...
        for (const [index, row] of jsonData.entries()) {
            ...
            try {
                ...
                const searchBox = await newPage.findElement(
                ...
                await newPage.wait(
                    until.elementLocated(
                        By.css("button[data-item-id='address']")
                    ),
                    10000
                );
