...
export const scrape = (req, res) => {
    ...
    if (!industry || !state || !website) {
        return res.status(400).json({ error: "Industry, state, and website are required." });
    }

    console.log(`Scraping started for industry: ${industry}, state: ${state}, website: ${website}`);

    let scriptPath;
    switch (website) {
        case "naukri":
            scriptPath = path.join(__dirname, "../scripts", "naukri_scraper.js");
            break;
        case "hirist":
            scriptPath = path.join(__dirname, "../scripts", "hirist_scraper.js");
            break;
        case "intern":
            scriptPath = path.join(__dirname, "../scripts", "intern_scraper.js");
            break;
        case "foundit":
            scriptPath = path.join(__dirname, "../scripts", "foundit_scraper.js");
            break;
        case "glassdoor":
            scriptPath = path.join(__dirname, "../scripts", "glassdoor_scraper.js");
            break;
        case "shine":
            scriptPath = path.join(__dirname, "../scripts", "shine_scraper.js");
            break;
        case "timesjob":
            scriptPath = path.join(__dirname, "../scripts", "timesjob_scraper.js");
            break;
        default:
            return res.status(400).json({ error: "Unsupported website selected." });
    }
    ...
    nodeProcess.on("close", async (code) => {
        ...
        res.json({
            ...
            data: rows.map(([job_title, company, location, address, phone, website, gst_number]) => ({
                job_title,
                company,
                location,
                address,
