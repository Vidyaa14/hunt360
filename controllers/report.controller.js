...
export const getReportSummary = async (req, res) => {
  try {
    ...
    const [[hrResult], [marketingResult], [editsResult]] = await Promise.all([
      db.query(hrQuery),
...
export const getLocationWiseLeadCount = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch location-wise lead count" });
  }
};
...
export const getStateWiseBdActivities = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch state-wise BD activities" });
  }
};
